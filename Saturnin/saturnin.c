 ...
 *   D     separation domain (0 to 15) ---> 6 for short
 *   key   key (32 bytes)
...
saturnin_block_encrypt(int R, int D, const uint8_t *key, uint8_t *buf)
{
	...
	for (i = 0; i < R; i ++) {
		...
		for(int i = 0; i < 16; ++i) printf("%x", xb[i]); printf(" Before SIGMA-1\n");
		S_box(xb);
		for(int i = 0; i < 16; ++i) printf("%x", xb[i]); printf(" After SIGMA-1\n");printf("\n");
		MDS(xb);

		/*
		 * Odd round.
		 */
		for(int i = 0; i < 16; ++i) printf("%x", xb[i]); printf(" Before SIGMA-2\n");
		S_box(xb);
		for(int i = 0; i < 16; ++i) printf("%x", xb[i]); printf(" After SIGMA-2\n");
		if ((i & 1) == 0) {
			/*
			 * Round r = 1 mod 4.
			 */
			...
			xb[0] ^= RC0[i];
			xb[8] ^= RC1[i];
			for(int i = 0; i < 16; ++i) printf("%x", xk[i]); printf(" This is the key\n");
...
// uint8_t buf[32] = {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F, 0x80}; 
uint8_t buf[32] = {0x00};
int main()
{
	for(int i = 0; i < 32; ++i) printf("%x", key[i]); printf("\n"); 
	for(int i = 0; i < 32; ++i) printf("%x", buf[i]); printf("\n"); 
	saturnin_block_encrypt(1,6,key,buf);
}