	...
	const unsigned char *k)
{
	...
	memcpy(tmp, npub, 16); // copy the nonce over for saturnin
	if (mlen > 0) {
		memcpy(tmp + 16, m, mlen); // copy the message over
	}
	tmp[16 + mlen] = 0x80; // pad the block
	memset(tmp + 16 + mlen + 1, 0x00, 15 - mlen); // set the rest of the padding to zero
