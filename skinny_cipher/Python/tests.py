...
class TestOfficialTestVectors:
    ...
    def key_to_bits(self, key):
        inter = str(int(bin(key)[2:])).zfill(8)
        key = '{:<0128s}'.format(inter)
        return key
    ...
    def generate_keys(self):
        ...
        keys = []
        for i in range(256):
            nkey = self.key_to_bits(i)
            keys.append(nkey)
        return keys

    def test_maahir(self):
        rows = []
        with open('plaintexts.csv') as csv_file:
            csv_reader = csv.reader(csv_file)
            for row in csv_reader:
                rowx = str(row).replace(" ", "").replace("['", "").replace("']","")
                rowx = bin(int(rowx, 16))[2:].zfill(128)
                rows.append(rowx)
        
        for row in rows:
            in_pt = int(hex(int(row,2)),16)
            keys = self.generate_keys()
            for key in keys:
                in_key = hex(int(key,2))
                in_key = int(hex(int(key,2)) + ('0' * (16-len(in_key[2:]))),16)
                p = SkinnyCipher(in_key, 128, 128)
                ct = p.encrypt(in_pt)
    ...
    def test_skinny_128_128(self):
        test_vec_128_128 = [[0x5ba8c7572ac2df5e1b474a91441abbc3, 0x454dad3782d7ffeb9eadef35f6920eea, 0x1ebd234bf43850e0acd14cd4f49d8ac6],
                            ...
                            [0x01bd8c480b87b76ee172ee2108817d86, 0x7f9511997a1a64342bbe8dbad5506e97, 0xe77e5d9a1fff3c7f9d8752eba9882f24]]
...
x = TestOfficialTestVectors()
print("testing skinny 128-128\n")
# x.test_skinny_128_128()
x.test_maahir()
# x.test_skinny_128_128()