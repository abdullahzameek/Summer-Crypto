...
class SkinnyCipher:
    ...
    def encrypt(self, plaintext):
        ...
        try:
            with open('output1.csv', mode='a', newline="") as csv_file:
                # writer = csv.writer(csv_file)
                # writer.writerow([hex(plaintext)+'\t'])
                
                csv_file.write(bin(int(hex(plaintext)[2:4],16))[2:].zfill(8)+'\t')
    ...
    def encrypt_function(self, internal_state):    
        ...
        # AddRoundConstant
        round_constant = self.round_constants[0]
        ...
        # AddTweakKey
        internal_state[0] = array('B', map(xor, internal_state[0], self.key_schedule[0][0]))
        internal_state[1] = array('B', map(xor, internal_state[1], self.key_schedule[0][1]))
        ...
        ct = self.state_to_int(internal_state)


        with open('output1.csv', mode='a', newline="") as csv_file:

            csv_file.write(hex(self.key)+'\t'+bin(int(hex(ct)[10:12],16))[2:].zfill(8) + '\n')
